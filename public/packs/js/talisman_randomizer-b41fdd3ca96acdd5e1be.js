...
/******/ 	return __webpack_require__(__webpack_require__.s = "./app/javascript/packs/talisman_randomizer.coffee");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./app/javascript/packs/talisman_randomizer.coffee":
/*!*********************************************************!*\
  !*** ./app/javascript/packs/talisman_randomizer.coffee ***!
  \*********************************************************/
...
/* WEBPACK VAR INJECTION */(function($) {var BOTH_ENDINGS, CHARACTERS, ENDINGS, HIDDEN_ONLY_ENDINGS, HIDE_PLAYERS, REVEALED_ONLY_ENDINGS, TOTAL_PLAYERS, TalismanRandomizer, shuffle, swap;
...
CHARACTERS = {
  base: ["Assassin", "Druid", "Dwarf", "Elf", "Ghoul", "Minstrel", "Monk", "Priest", "Prophetess", "Sorceress", "Troll", "Thief", "Wizard", "Warrior"],
  ...
  nether_realm: [],
  realms_unraveled: [],
  digital_edition: ["Pirate", "Ninja", "Exorcist", "Courtesan", "Devil's Minion", "Genie", "Martyr", "Gambler", "Black Witch", "Apprentice Mage", "Shape Shifter", "Shaman", "Illusionist", "Jester", "Goblin Shaman", "Woodsman", "Pathfinder", "Samurai", "Martial Artist", "Witch Doctor", "Sacaren"]
};
...
TalismanRandomizer = {
  ...
  randomize_all_player_buildings: function() {
    var PLAYER_BUILDINGS, k, results, x;
    PLAYER_BUILDINGS = shuffle(PLAYER_BUILDINGS);
    results = [];
    for (x = k = 1; k <= 6; x = ++k) {
      results.push(TinyTownsRandomizer.set_player_buildings(x));
    }
    return results;
  },
  randomize_player_buildings: function(number) {
    ...
    return TinyTownsRandomizer.set_player_buildings(number);
  },
  set_player_buildings: function(number) {
    ...
    return elements[1].getElementsByClassName("text")[0].innerHTML = PLAYER_BUILDINGS[number * 2 - 1];
  },
  toggle_all_player_buildings: function() {
    ...
    if (HIDE_PLAYERS) {
      results = [];
      for (x = k = 1; k <= 6; x = ++k) {
        results.push(TinyTownsRandomizer.hide_player_buildings(x));
      }
      return results;
    } else {
      results1 = [];
      for (x = l = 1; l <= 6; x = ++l) {
        results1.push(TinyTownsRandomizer.reveal_player_buildings(x));
      }
      return results1;
    }
  },
  toggle_player_buildings: function(number) {
    return $("#player-" + number + " .purple").toggleClass("unrevealed");
  },
  reveal_player_buildings: function(number) {
    return $("#player-" + number + " .purple").removeClass("unrevealed");
  },
  hide_player_buildings: function(number) {
    return $("#player-" + number + " .purple").addClass("unrevealed");
  },
  randomize_until_different_building: function(color) {
    var element, old_building;
    element = document.getElementsByClassName(color)[0].getElementsByClassName("text")[0];
    old_building = element.innerHTML;
    while (old_building === TinyTownsRandomizer.randomize_building(color)) {
      continue;
    }
    return element.innerHTML;
  },
  randomize_all: function() {
    ...
    return TinyTownsRandomizer.change_total_players(4);
  },
  randomize_all_buildings: function() {
    ...
    for (x in BUILDINGS) {
      y = BUILDINGS[x];
      results.push(TinyTownsRandomizer.randomize_building(x));
    }
    return results;
  },
  change_total_players: function(number) {
    ...
    results = [];
    for (x = k = 1; k <= 6; x = ++k) {
      results.push(TinyTownsRandomizer.toggle_player(x));
    }
    return results;
  },
...
$(document).on("click", "[data-behavior~=randomize-building]", function(event) {
  ...
  return TinyTownsRandomizer.randomize_until_different_building(color);
});
...
$(document).on("click", "[data-behavior~=randomize-player]", function(event) {
  ...
  return TinyTownsRandomizer.randomize_player_buildings(number);
});
...
$(document).on("click", "[data-behavior~=reveal-player]", function(event) {
  ...
  event.preventDefault();
  number = this.getAttribute('data-number');
  return TinyTownsRandomizer.toggle_player_buildings(number);
});

$(document).on("click", "[data-behavior~=reveal-all-players]", function(event) {
  event.preventDefault();
  return TinyTownsRandomizer.toggle_all_player_buildings();
});

$(document).on("click", "[data-behavior~=randomize-all-players]", function(event) {
  event.preventDefault();
  return TinyTownsRandomizer.randomize_all_player_buildings();
});

$(document).on("click", "[data-behavior~=randomize-all-buildings]", function(event) {
  event.preventDefault();
  return TinyTownsRandomizer.randomize_all_buildings();
});

$(document).ready(function() {
  return TinyTownsRandomizer.randomize_all();
});

$(document).on("click", "input[type=radio][name=total-players]", function(event) {
  var number;
  number = this.getAttribute('value');
  return TinyTownsRandomizer.change_total_players(number);
});
...
/******/ });
//# sourceMappingURL=talisman_randomizer-b41fdd3ca96acdd5e1be.js.map